...
# uart = serial.Serial('COM3', 921600)  # 在 windows 下控制电机测试，相应的输入连接的COM口和波特率
...
uart = serial.Serial(com,921600 , timeout=0.5)
uart.flush()

result1 = os.popen("sudo ls -l /dev/ttyUSB1").read()  # 在 jetson nano（ubuntu）及树莓派（raspbian）下控制电机测试，相应的输入连接的串口和波特率
com1 = result1.split()[-1]  # sudo chmod 666 /dev/ttyACM* 有时会报错，显示无曲线打开串口，这时需运行左侧命令
os.system("sudo chmod 777 " + com1)
# os.system("sudo fuser -k " + com)
uart1 = serial.Serial(com1,921600 , timeout=0.5)
uart1.flush()
...
# ttyUSB0串口发送函数
def write_data1(data=[]):
    global uart1
    if uart1.inWaiting() > 0:
        uart1.read(uart1.inWaiting())
    try:
        result = uart1.write(data)  # 写数据
        # print("write_data, result ",data)
        return result
    except Exception as e:
        print("---error in write_data--：", e)
...
def read_data1():
    ...
    i = 250 # 经过测试，发现正常接收16位耗时大概为500
    # 该函数中的两处time.sleep(0.001)会影响CAN接收速度，如果要提高CAN读取速度，可以将这两行注释掉，并将上面的i改成一个较大的值，比如5000
    while uart1.inWaiting() == 0 and i > 0:  # To do:
        i -= 1
    #     time.sleep(0.001)
    # time.sleep(0.001)
    while uart1.inWaiting() > 0:
        byte_list.append(list(uart1.read(1))[0])
    if len(byte_list) != 0:
        READ_FLAG = 1
        return byte_list
    else:
...
def organize_can_message2(id_num=127, cmd_mode=0, cmd_data=[], data=[], data_num = 0x08, rtr=0):
    cdata = [0x00, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
    cdata[0] = ((cmd_mode << 3) | (cmd_data[0] >> 5))& 0xff
    cdata[1] = ((cmd_data[0] << 3) | (cmd_data[1] >> 5))& 0xff
    cdata[2] = ((cmd_data[1] << 3) | (id_num >>5)) & 0xff    # master_ID
    cdata[3] =( (id_num << 3) | 0x04)& 0xff
    cdata[4] = data_num
    for i in range(8):
        cdata[5 + i] = data[i]
    return cdata
...
def send_command(id_num=127, cmd_mode=0, cmd_data=[], data=[], data_num = 0x08, rtr=0):
    ...
    write_data1(data=can_to_uart(data=cdata, rtr=rtr))
...
def receive_data1():   
    udata1 = read_data1()
    print("receive_data udata", udata1)
    if READ_FLAG == 1:
        cdata = uart_to_can(data=udata1)
        if cdata[1] == 21:
            dump_error(cdata, True)
        return cdata[:]
...
def impedance_control(tff=0, id_num=127, pos=0, vel=0, kp=0, kd=0):
    """运控模式电机控制指令函数。
    ...
    try:
        ...
        cmd_data = [0] * 2
        tff_value = int(((tff + 12.0) / 24.0) * 65535)
        cmd_data[0] = (tff_value >> 8) & 0xFF  # 阻抗控制里都是高位在前，低位在后，不满足小端排序
        cmd_data[1] = tff_value & 0xFF
        # print(cmd_data[0])
        # print(cmd_data[1])

        tx_data = [0] * 8
        # Byte6-7: 目标角度 [0~65535] 对应 (-4π~4π)
        target_angle_value = int((pos + 12.5) / 25 * 65535)
        tx_data[0] = (target_angle_value >> 8) & 0xFF
        tx_data[1] = target_angle_value & 0xFF

        # Byte4-5: 目标角速度 [0~65535] 对应 (-30 rad/s 到 30 rad/s)
        target_velocity_value = int((vel + 30) / 60 * 65535)
        tx_data[2] = (target_velocity_value >> 8) & 0xFF
        tx_data[3] = target_velocity_value & 0xFF

        # Byte2-3: Kp [0~65535] 对应 (0.0 到 500.0)
        kp_value = int((kp / 500.0) * 65535)
        tx_data[4] = (kp_value >> 8) & 0xFF
        tx_data[5] = kp_value & 0xFF

        # Byte0-1: Kd [0~65535] 对应 (0.0 到 5.0)
        kd_value = int((kd / 5.0) * 65535)
        tx_data[6] = (kd_value >> 8) & 0xFF
        tx_data[7] = kd_value & 0xFF

        send_command(id_num=id_num, cmd_mode=1, cmd_data=cmd_data, data=tx_data, data_num=0x08,rtr=0)  # 需要用扩展帧（数据帧）进行发送
        reply_state(id_num=id_num)
    except Exception as e:
        print("---error in impedance_contro--：", e)
...
def set_at_mode():
    ...
    write_data1([0x41, 0x54, 0x2B, 0x41, 0x54, 0x0D, 0x0A])
    recived_data1 = read_data1()

    for i in range(len(recived_data1)):
        recived_chr[i] = chr(recived_data1[i])
    # print("set AT mode", str(recived_chr))                                    #调试
...
def set_zero(i,j):
    set_zero_position(id_num=i)
    set_zero_position(id_num=j)
    time.sleep(0.1)
    set_angle(id_num=i, angle=0, speed=20, limit_cur=23)
    set_angle(id_num=j, angle=0, speed=20, limit_cur=23)
    time.sleep(0.1)
    motor_estop(id_num=i)
    motor_estop(id_num=j)




# 将每一行转换成所需格式并发送
tdata= [0x41, 0x54, 0x2b, 0x41, 0x54,0x0d, 0x0a]
uart.write(tdata)
lines = [
    ...
    "41 54 00 07 e9 8c 01 00 0d 0a",
    "41 54 00 07 e9 cc 01 00 0d 0a",
    "41 54 00 07 ea 0c 01 00 0d 0a",
    "41 54 00 07 ea 4c 01 00 0d 0a",
    "41 54 00 07 ea 8c 01 00 0d 0a",
    "41 54 00 07 ea cc 01 00 0d 0a",
    "41 54 00 07 eb 0c 01 00 0d 0a",
    "41 54 00 07 eb 4c 01 00 0d 0a",
...
for line in lines:
    # 将十六进制字符串分割并转换为整数列表
    tdata = [int(x, 16) for x in line.split()]
    # print(data)
    # 发送数据
    uart.write(tdata)
...
# set_zero_position(id_num=2)
# set_zero_position(id_num=9)
# time.sleep(0.1)
# set_angle(id_num=2, angle=90, speed=20, limit_cur=23)
# set_angle(id_num=9, angle=30, speed=20, limit_cur=23)
# time.sleep(5)
